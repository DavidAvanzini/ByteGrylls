...
|--------------|-------------------------------------------------|-----------------------------|
| `nc`         | Test whether a TCP port is open (client mode)   | None                        |
| `listen`     | Open a local TCP listener (server mode)         | None                        |
| `dns`        | Resolve an A record via raw UDP DNS query        | None                        |
| `ping`       | Send raw ICMP Echo Requests                      | root/sudo or Administrator  |
| `traceroute` | Trace the route to a host via increasing TTL     | root/sudo or Administrator  |

---

### `nc` — test a TCP port (Netcat client mode)

Use it exactly like you'd use `nc -zv` to check if something is listening.
